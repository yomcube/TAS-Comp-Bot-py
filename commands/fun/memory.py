...
class Memory(commands.Cog):
    ...
    async def command(self, ctx, size: int = 4):
        if size % 2 != 0:
            await ctx.send("Board size must be an even number!")
            return
        ...
        embed = discord.Embed(
            ...
            description=f"Welcome to the memory game!\n\n"
                        "In this game, a board is presented to you and need to match all pairs of emojies to win! "
        ...
        try:
            ...
            if len(guild_emojis) < size * size // 2:
            ...
            unique_emojis = random.sample(guild_emojis, size * size // 2)
            ...
            display_board = [[board_emojis.pop() for _ in range(size)] for _ in range(size)]
            ...
            def reveal_area(row, col, radius=1):
                ...
                for r in range(max(0, row - radius), min(size, row + radius + 1)):
                    for c in range(max(0, col - radius), min(size, col + radius + 1)):
                        if hidden_board[r][c] == '❓':  # Only reveal hidden tiles
            ...
            def keep_only_selected_visible(row, col):
                single_tile_board = [['❓' for _ in range(size)] for _ in range(size)]
                for r in range(size):
                    for c in range(size):
                        if hidden_board[r][c] != '❓':  # Keep matched pairs visible
            ...
            while lives > 0 and len(found_pairs) < size * size:
                try:
                    # Get first choice from the user with a 60-second timeout
                    await ctx.send(f"Select first tile! Lives remaining: {lives}")
                    msg = await self.bot.wait_for('message', timeout=60.0, check=lambda m: m.author == ctx.author and is_valid_choice(m.content))
                    ...
                    # Get second choice from the user with a 60-second timeout
            ...
            elif len(found_pairs) == size * size:
